...
#define ICON_PX 64
#define META_POPUP_PIN_THRESHOLD_PX 8
...
static EFI_STATUS ensure_meta_dir(EFI_FILE_HANDLE root, CHAR16 *dir){
    EFI_FILE_HANDLE mh; CHAR16 mp[320]; EFI_STATUS s;
    if(StrCmp(dir,L"\\")==0) StrCpy(mp,L"\\.meta");
    else SPrint(mp,sizeof(mp),L"%s\\.meta",dir);
    s=uefi_call_wrapper(root->Open,5,root,&mh,mp,EFI_FILE_MODE_READ|EFI_FILE_MODE_WRITE|EFI_FILE_MODE_CREATE,EFI_FILE_DIRECTORY);
    if(EFI_ERROR(s)) return s;
    uefi_call_wrapper(mh->Close,1,mh);
    return EFI_SUCCESS;
}
#ifndef SCAN_PRINT
...
static EFI_STATUS rename_selected_with_meta(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *cwd, CHAR16 *old_name, CHAR16 *new_name){
    ...
    if(!EFI_ERROR(s)){
        make_meta_path_in_dir(om,sizeof(om),cwd,old_name); make_meta_path_in_dir(nm,sizeof(nm),cwd,new_name);
...
static EFI_STATUS move_selected_with_meta(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *src_cwd, CHAR16 *name, CHAR16 *dst_cwd){
    EFI_FILE_HANDLE root; EFI_STATUS s; CHAR16 src[320],dst[320],sm[360],dm[360];
    if(StrCmp(src_cwd,dst_cwd)==0) return EFI_ALREADY_STARTED;
    join_path(src,sizeof(src),src_cwd,name);
    join_path(dst,sizeof(dst),dst_cwd,name);
    s=open_root(ih,st,&root); if(EFI_ERROR(s)) return s;
    s=rename_in_dir(root,src,dst);
    if(!EFI_ERROR(s)){
        ...
        (void)rename_in_dir(root,sm,dm);
    }
    uefi_call_wrapper(root->Close,1,root);
    return s;
}
static EFI_STATUS copy_path_file(EFI_FILE_HANDLE root, CHAR16 *src_path, CHAR16 *dst_path){
    EFI_FILE_HANDLE src,dst; EFI_STATUS s; UINT8 *buf; UINTN chunk=65536,rd,wr;
    s=uefi_call_wrapper(root->Open,5,root,&src,src_path,EFI_FILE_MODE_READ,0); if(EFI_ERROR(s)) return s;
    s=uefi_call_wrapper(root->Open,5,root,&dst,dst_path,EFI_FILE_MODE_READ,0);
    if(!EFI_ERROR(s)){ uefi_call_wrapper(dst->Close,1,dst); uefi_call_wrapper(src->Close,1,src); return EFI_ACCESS_DENIED; }
    s=uefi_call_wrapper(root->Open,5,root,&dst,dst_path,EFI_FILE_MODE_READ|EFI_FILE_MODE_WRITE|EFI_FILE_MODE_CREATE,0);
    if(EFI_ERROR(s)){ uefi_call_wrapper(src->Close,1,src); return s; }
    buf=AllocatePool(chunk); if(buf==NULL){ uefi_call_wrapper(dst->Delete,1,dst); uefi_call_wrapper(src->Close,1,src); return EFI_OUT_OF_RESOURCES; }
    while(1){ rd=chunk; s=uefi_call_wrapper(src->Read,3,src,&rd,buf); if(EFI_ERROR(s)||rd==0) break; wr=rd; s=uefi_call_wrapper(dst->Write,3,dst,&wr,buf); if(EFI_ERROR(s)||wr!=rd){ s=EFI_DEVICE_ERROR; break; } }
    FreePool(buf); uefi_call_wrapper(src->Close,1,src);
    if(EFI_ERROR(s)){ uefi_call_wrapper(dst->Delete,1,dst); return s; }
...
static EFI_STATUS run_item_with_args(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *cwd, CHAR16 *prog, CHAR16 *arg){ EFI_LOADED_IMAGE *li,*child=NULL; EFI_DEVICE_PATH *dp; EFI_HANDLE h=NULL; EFI_STATUS s; CHAR16 path[180]; s=uefi_call_wrapper(st->BootServices->HandleProtocol,3,ih,&LoadedImageProtocol,(VOID**)&li); if(EFI_ERROR(s)) return s; join_path(path,sizeof(path),cwd,prog); dp=FileDevicePath(li->DeviceHandle,path); if(dp==NULL) return EFI_NOT_FOUND; s=uefi_call_wrapper(st->BootServices->LoadImage,6,FALSE,ih,dp,NULL,0,&h); if(EFI_ERROR(s) && StrCmp(cwd,L"\\")!=0){ SPrint(path,sizeof(path),L"\\%s",prog); dp=FileDevicePath(li->DeviceHandle,path); if(dp==NULL) return EFI_NOT_FOUND; s=uefi_call_wrapper(st->BootServices->LoadImage,6,FALSE,ih,dp,NULL,0,&h); } if(EFI_ERROR(s)) return s; if(arg!=NULL && arg[0]!=0){ s=uefi_call_wrapper(st->BootServices->HandleProtocol,3,h,&LoadedImageProtocol,(VOID**)&child); if(!EFI_ERROR(s) && child!=NULL){ child->LoadOptions=(VOID*)arg; child->LoadOptionsSize=(UINT32)((StrLen(arg)+1)*sizeof(CHAR16)); } } return uefi_call_wrapper(st->BootServices->StartImage,3,h,NULL,NULL); }
...
static VOID read_meta_type_handler(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *cwd, CHAR16 *name, CHAR8 *type, UINTN type_len, CHAR8 *handler, UINTN handler_len){ EFI_FILE_HANDLE root,f; CHAR16 path[300]; UINT8 buf[1024]; UINTN sz=sizeof(buf)-1,i=0,j; EFI_STATUS s; type[0]=0; handler[0]=0; s=open_root(ih,st,&root); if(EFI_ERROR(s)) return; make_meta_path_in_dir(path,sizeof(path),cwd,name); s=uefi_call_wrapper(root->Open,5,root,&f,path,EFI_FILE_MODE_READ,0); if(EFI_ERROR(s)){ uefi_call_wrapper(root->Close,1,root); return; } s=uefi_call_wrapper(f->Read,3,f,&sz,buf); uefi_call_wrapper(f->Close,1,f); uefi_call_wrapper(root->Close,1,root); if(EFI_ERROR(s)||sz==0) return; buf[sz]=0; while(i<sz){ j=i; while(j<sz && buf[j]!='\n' && buf[j]!='\r') j++; if(j>i){ if(j-i>6 && contains_ci((CHAR8*)&buf[i],(const CHAR8*)"TYPE:")){ UINTN k=0,p=i+5; while(p<j && (buf[p]==' '||buf[p]=='\t')) p++; while(p<j && k+1<type_len) type[k++]=buf[p++]; type[k]=0; } if(j-i>9 && contains_ci((CHAR8*)&buf[i],(const CHAR8*)"HANDLER:")){ UINTN k=0,p=i+8; while(p<j && (buf[p]==' '||buf[p]=='\t')) p++; while(p<j && k+1<handler_len) handler[k++]=buf[p++]; handler[k]=0; } } while(j<sz && (buf[j]=='\n'||buf[j]=='\r')) j++; i=j; } }


static VOID read_meta_preview(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *cwd, CHAR16 *name, CHAR16 *out, UINTN out_len){
    EFI_FILE_HANDLE root,f; CHAR16 path[220]; UINT8 buf[256]; UINTN sz=sizeof(buf)-1,i=0,o=0; EFI_STATUS s;
    if(out_len==0) return;
    out[0]=0;
    s=open_root(ih,st,&root); if(EFI_ERROR(s)) return;
    make_meta_path_in_dir(path,sizeof(path),cwd,name);
    s=uefi_call_wrapper(root->Open,5,root,&f,path,EFI_FILE_MODE_READ,0);
    if(EFI_ERROR(s)){ uefi_call_wrapper(root->Close,1,root); StrCpy(out,L"No metadata sidecar"); return; }
    s=uefi_call_wrapper(f->Read,3,f,&sz,buf); uefi_call_wrapper(f->Close,1,f); uefi_call_wrapper(root->Close,1,root);
    if(EFI_ERROR(s)||sz==0){ StrCpy(out,L"Metadata empty"); return; }
    buf[sz]=0;
    while(i<sz && o+1<out_len){
        if(buf[i]=='\r'){ i++; continue; }
        out[o++]=(CHAR16)(buf[i]&0xFF);
        i++;
    }
    out[o]=0;
}
...
static UINTN meta_preview_line(CHAR16 *src, UINTN start, CHAR16 *out, UINTN out_len){
    UINTN i=start,o=0;
    if(out_len==0) return start;
    while(src[i] && src[i]!=L'\n' && o+1<out_len){ out[o++]=src[i++]; }
    out[o]=0;
    if(src[i]==L'\n') i++;
    return i;
}
static EFI_STATUS write_meta_popup_text(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, CHAR16 *cwd, CHAR16 *name, CHAR16 *text){
    EFI_FILE_HANDLE root,mf; EFI_STATUS s; CHAR16 path[320]; CHAR8 buf[META_BUF]; UINTN i=0,n=0,sz;
    s=open_root(ih,st,&root); if(EFI_ERROR(s)) return s;
    make_meta_path_in_dir(path,sizeof(path),cwd,name);
    s=uefi_call_wrapper(root->Open,5,root,&mf,path,EFI_FILE_MODE_READ|EFI_FILE_MODE_WRITE|EFI_FILE_MODE_CREATE,0);
    if(EFI_ERROR(s)){ uefi_call_wrapper(root->Close,1,root); return s; }
    while(text && text[i] && n+1<sizeof(buf)){ buf[n++]=(text[i]<=127)?(CHAR8)text[i]:'?'; i++; }
    buf[n]=0; sz=n;
    (void)uefi_call_wrapper(mf->SetPosition,2,mf,0);
    s=uefi_call_wrapper(mf->Write,3,mf,&sz,buf);
    uefi_call_wrapper(mf->Close,1,mf); uefi_call_wrapper(root->Close,1,root);
    return s;
}
...
static VOID compute_meta_popup_bounds(TT_FONT *ttfont, CHAR16 *popup_text, INTN popup_x, INTN popup_y, UINTN max_w, UINTN max_h, UINTN *out_x, UINTN *out_y, UINTN *out_w, UINTN *out_h){
    UINTN li=0,pos=0,maxw=0,pw=180,ph=48,px2=(popup_x<0)?0:(UINTN)popup_x,py2=(popup_y<0)?0:(UINTN)popup_y;
    CHAR16 lines[6][96];
    ZeroMem(lines,sizeof(lines));
    while(li<6 && popup_text && popup_text[pos]){
    ...
    if(px2+pw>max_w) px2=(max_w>pw)?(max_w-pw):0;
    if(py2+ph>max_h) py2=(max_h>ph)?(max_h-ph):0;
    if(out_x) *out_x=px2;
    if(out_y) *out_y=py2;
    if(out_w) *out_w=pw;
    if(out_h) *out_h=ph;
}
static UINTN rename_click_to_cursor(TT_FONT *tt, CHAR16 *text, UINTN rel_x){
    UINTN len=StrLen(text),i,best=0,best_dist=(UINTN)-1;
    for(i=0;i<=len;i++){
        CHAR16 tmp[LAUNCHER_NAME_MAX]; UINTN w,dist;
        StrnCpy(tmp,text,i); tmp[i]=0;
        w=tt_text_width(tt,tmp);
        dist=(w>rel_x)?(w-rel_x):(rel_x-w);
...
static VOID draw_window(RENDER_TARGET *rt, EFI_GRAPHICS_OUTPUT_PROTOCOL *g, TT_FONT *ttfont, LAUNCHER_WINDOW *w, ICON_IMAGE *close_img, BOOLEAN show_close, EFI_GRAPHICS_OUTPUT_BLT_PIXEL win, EFI_GRAPHICS_OUTPUT_BLT_PIXEL title, EFI_GRAPHICS_OUTPUT_BLT_PIXEL selc, EFI_GRAPHICS_OUTPUT_BLT_PIXEL btn, EFI_GRAPHICS_OUTPUT_BLT_PIXEL white, BOOLEAN rename_mode, CHAR16 *rename_buf, UINTN rename_cursor, BOOLEAN meta_popup, INTN popup_x, INTN popup_y, CHAR16 *popup_text, BOOLEAN drag_target, BOOLEAN hide_drag_item, UINTN drag_item_idx, UINTN folder_target_idx){
    ...
    fill(rt,g,w->x,w->y,w->w,2,title); fill(rt,g,w->x,w->y+w->h-2,w->w,2,title);
    fill(rt,g,w->x,w->y,2,w->h,title); fill(rt,g,w->x+w->w-2,w->y,2,w->h,title);
    if(drag_target){
    ...
    if(folder_target_idx!=(UINTN)-1 && folder_target_idx>=start && folder_target_idx<end){
        UINTN vi=folder_target_idx-start,row=vi/w->cols,col=vi%w->cols;
        UINTN ix=w->x+12+col*90+26/2, iy=w->content_y+row*83, iw=ICON_PX, ih=ICON_PX+16;
        EFI_GRAPHICS_OUTPUT_BLT_PIXEL yl={40,220,255,0};
        fill(rt,g,ix-2,iy-2,iw+4,2,yl); fill(rt,g,ix-2,iy+ih,iw+4,2,yl);
        fill(rt,g,ix-2,iy-2,2,ih+4,yl); fill(rt,g,ix+iw,iy-2,2,ih+4,yl);
    }
    if(meta_popup && popup_text && popup_text[0]){
        ...
        compute_meta_popup_bounds(ttfont,popup_text,popup_x,popup_y,rt->w,rt->h,&px2,&py2,&pw,&ph);
        li=0;
...
static VOID update_window_grid_metrics(LAUNCHER_WINDOW *w){
    UINTN usable_w=(w->w>26)?(w->w-26):w->w;
    w->cols=usable_w/90; if(w->cols<1) w->cols=1;
    w->rows=(w->count+w->cols-1)/w->cols;
    w->visible_rows=w->content_h/83; if(w->visible_rows<1) w->visible_rows=1;
    w->max_scroll=(w->rows>w->visible_rows)?(w->rows-w->visible_rows):0;
    if(w->scroll>w->max_scroll) w->scroll=w->max_scroll;
}
static VOID reload_window(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, LAUNCHER_WINDOW *w){ UINTN i; free_window_icons(st,w); w->count=load_items(ih,st,w->items,w->cwd); for(i=0;i<w->count;i++) load_icon(ih,st,w->items[i].icon,&w->icons[i]); update_window_grid_metrics(w); }
static VOID reload_all_windows(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st, LAUNCHER_WINDOW *wins, UINTN win_count){ UINTN i; for(i=0;i<win_count;i++) reload_window(ih,st,&wins[i]); }
static VOID layout_window(LAUNCHER_WINDOW *w){ w->content_y=w->y+36; w->content_h=(w->h>46)?(w->h-46):1; update_window_grid_metrics(w); }

static INTN bring_window_to_front(LAUNCHER_WINDOW *wins, UINTN win_count, INTN wi){ UINTN i; LAUNCHER_WINDOW t; if(wi<0||(UINTN)wi>=win_count) return -1; if((UINTN)wi==win_count-1) return wi; t=wins[wi]; for(i=(UINTN)wi;i+1<win_count;i++) wins[i]=wins[i+1]; wins[win_count-1]=t; return (INTN)win_count-1; }
EFI_STATUS efi_main(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st){ EFI_STATUS s; RENDER_TARGET rt; EFI_GRAPHICS_OUTPUT_PROTOCOL *g=NULL; EFI_GUID gg=gEfiGraphicsOutputProtocolGuid; EFI_SIMPLE_POINTER_PROTOCOL *sp=NULL; EFI_GUID spg=EFI_SIMPLE_POINTER_PROTOCOL_GUID; EFI_ABSOLUTE_POINTER_PROTOCOL *ap=NULL; EFI_GUID apg=EFI_ABSOLUTE_POINTER_PROTOCOL_GUID; EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *inex=NULL; EFI_GUID inexg=EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL_GUID; EFI_INPUT_KEY k; TT_FONT ttfont; EFI_EVENT events[4]; UINTN evn=0,which=0; POINTER_IMAGE ptr_img; ICON_IMAGE close_img; EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ptr_bg=NULL; UINTN ptr_w=12,ptr_h=14; BOOLEAN ptr_bg_valid=FALSE; INTN px=200,py=140,ppx=-1,ppy=-1; BOOLEAN prev_left=FALSE,need_redraw=TRUE,rename_mode=FALSE,rename_all_selected=FALSE; CHAR16 rename_buf[LAUNCHER_NAME_MAX]; UINTN rename_cursor=0; CHAR16 toast[64]; UINTN toast_ticks=0; BOOLEAN meta_popup=FALSE; CHAR16 meta_popup_text[192]; INTN meta_popup_x=0,meta_popup_y=0; BOOLEAN prev_right=FALSE; BOOLEAN meta_popup_pin_armed=FALSE,meta_popup_pinned=FALSE,meta_popup_edit_mode=FALSE,meta_popup_dirty=FALSE; INTN meta_popup_open_px=0,meta_popup_open_py=0; UINTN meta_popup_cursor=0; CHAR16 meta_popup_cwd[256],meta_popup_name[LAUNCHER_NAME_MAX];
 ...
 LAUNCHER_WINDOW wins[MAX_WINDOWS]; UINTN win_count=1; INTN active=0; UINTN i; BOOLEAN dragging=FALSE,resizing=FALSE; INTN drag_dx=0,drag_dy=0; UINTN resize_edge=0,ox=0,oy=0,ow=0,oh=0; BOOLEAN partial_redraw=FALSE; UINTN pr_old_x=0,pr_old_y=0,pr_old_w=0,pr_old_h=0,pr_new_x=0,pr_new_y=0,pr_new_w=0,pr_new_h=0; BOOLEAN item_dragging=FALSE,pending_item_drag=FALSE,drag_copy=FALSE; INTN drag_src_win=-1; UINTN drag_item=(UINTN)-1; INTN drag_hover_win=-1; UINTN drag_hover_item=(UINTN)-1; INTN drag_hot_x=ICON_PX/2,drag_hot_y=ICON_PX/2,drag_start_px=0,drag_start_py=0;
 InitializeLib(ih,st); disable_uefi_watchdog(st); ZeroMem(&rt,sizeof(rt)); toast[0]=0; meta_popup_cwd[0]=0; meta_popup_name[0]=0; s=uefi_call_wrapper(st->BootServices->LocateProtocol,3,&gg,NULL,(VOID**)&g); if(EFI_ERROR(s)) return s; rt.w=g->Mode->Info->HorizontalResolution; rt.h=g->Mode->Info->VerticalResolution; rt.buf=AllocatePool(rt.w*rt.h*sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL)); rt.use_backbuf=(rt.buf!=NULL)?TRUE:FALSE; uefi_call_wrapper(st->BootServices->LocateProtocol,3,&spg,NULL,(VOID**)&sp); uefi_call_wrapper(st->BootServices->LocateProtocol,3,&apg,NULL,(VOID**)&ap); uefi_call_wrapper(st->BootServices->LocateProtocol,3,&inexg,NULL,(VOID**)&inex);
 ...
 s=uefi_call_wrapper(st->BootServices->WaitForEvent,3,evn,events,&which); if(EFI_ERROR(s)) break; if(which==0){ if(inex){ EFI_KEY_DATA kd; if(!EFI_ERROR(uefi_call_wrapper(inex->ReadKeyStrokeEx,2,inex,&kd))){ UINT32 sh=kd.KeyState.KeyShiftState; if(kd.Key.ScanCode==SCAN_ESC && (sh&EFI_SHIFT_STATE_VALID) && (sh&(EFI_RIGHT_CONTROL_PRESSED|EFI_LEFT_CONTROL_PRESSED)) && (sh&(EFI_RIGHT_ALT_PRESSED|EFI_LEFT_ALT_PRESSED))) break; if(kd.Key.ScanCode==SCAN_F12 || kd.Key.ScanCode==SCAN_PRINT){ if(!EFI_ERROR(save_screenshot_bmp(ih,st,g))){ UINTN rw; StrCpy(toast,L"Saved \\screenshot"); for(rw=0;rw<win_count;rw++) reload_window(ih,st,&wins[rw]); } else StrCpy(toast,L"Screenshot failed"); toast_ticks=120; need_redraw=TRUE; } else if(meta_popup && meta_popup_edit_mode){ UINTN len=StrLen(meta_popup_text); if(kd.Key.ScanCode==SCAN_ESC){ meta_popup_edit_mode=FALSE; need_redraw=TRUE; } else if(kd.Key.ScanCode==SCAN_LEFT){ if(meta_popup_cursor>0) meta_popup_cursor--; need_redraw=TRUE; } else if(kd.Key.ScanCode==SCAN_RIGHT){ if(meta_popup_cursor<len) meta_popup_cursor++; need_redraw=TRUE; } else if(kd.Key.UnicodeChar==CHAR_BACKSPACE){ if(meta_popup_cursor>0){ CopyMem(&meta_popup_text[meta_popup_cursor-1],&meta_popup_text[meta_popup_cursor],(len-meta_popup_cursor+1)*sizeof(CHAR16)); meta_popup_cursor--; meta_popup_dirty=TRUE; } need_redraw=TRUE; } else if(kd.Key.UnicodeChar>=32 && kd.Key.UnicodeChar<127){ if(len+1<192){ CopyMem(&meta_popup_text[meta_popup_cursor+1],&meta_popup_text[meta_popup_cursor],(len-meta_popup_cursor+1)*sizeof(CHAR16)); meta_popup_text[meta_popup_cursor]=kd.Key.UnicodeChar; meta_popup_cursor++; meta_popup_dirty=TRUE; } need_redraw=TRUE; } } else if(rename_mode && active>=0){ LAUNCHER_WINDOW *aw=&wins[active]; UINTN len=StrLen(rename_buf); if(kd.Key.ScanCode==SCAN_ESC){ rename_mode=FALSE; StrCpy(toast,L"Rename canceled"); toast_ticks=120; need_redraw=TRUE; } else if(kd.Key.ScanCode==SCAN_LEFT){ if(rename_cursor>0) rename_cursor--; rename_all_selected=FALSE; need_redraw=TRUE; } else if(kd.Key.ScanCode==SCAN_RIGHT){ if(rename_cursor<len) rename_cursor++; rename_all_selected=FALSE; need_redraw=TRUE; } else if(kd.Key.UnicodeChar==CHAR_BACKSPACE){ if(rename_all_selected){ rename_buf[0]=0; rename_cursor=0; rename_all_selected=FALSE; } else if(rename_cursor>0){ CopyMem(&rename_buf[rename_cursor-1],&rename_buf[rename_cursor],(len-rename_cursor+1)*sizeof(CHAR16)); rename_cursor--; } need_redraw=TRUE; } else if(kd.Key.UnicodeChar==CHAR_CARRIAGE_RETURN){ if(aw->sel<aw->count && !EFI_ERROR(rename_selected_with_meta(ih,st,aw->cwd,aw->items[aw->sel].name,rename_buf))){ reload_all_windows(ih,st,wins,win_count); StrCpy(toast,L"Rename success"); } else StrCpy(toast,L"Rename failed"); toast_ticks=120; rename_mode=FALSE; rename_all_selected=FALSE; need_redraw=TRUE; } else if(kd.Key.UnicodeChar>=32 && kd.Key.UnicodeChar<127){ if(rename_all_selected){ rename_buf[0]=0; rename_cursor=0; len=0; rename_all_selected=FALSE; } if(len+1<LAUNCHER_NAME_MAX){ CopyMem(&rename_buf[rename_cursor+1],&rename_buf[rename_cursor],(len-rename_cursor+1)*sizeof(CHAR16)); rename_buf[rename_cursor]=kd.Key.UnicodeChar; rename_cursor++; } need_redraw=TRUE; } } else if(kd.Key.ScanCode==SCAN_DELETE && active>=0){ LAUNCHER_WINDOW *aw=&wins[active]; if(aw->sel<aw->count && StrCmp(aw->items[aw->sel].name,L"..")!=0){ if(!EFI_ERROR(delete_selected_with_meta(ih,st,aw->cwd,aw->items[aw->sel].name))){ reload_all_windows(ih,st,wins,win_count); if(aw->count==0) aw->sel=0; else if(aw->sel>=aw->count) aw->sel=aw->count-1; StrCpy(toast,L"Delete success"); toast_ticks=120; need_redraw=TRUE; } else { StrCpy(toast,L"Delete failed"); toast_ticks=120; need_redraw=TRUE; } } } } } else if(!EFI_ERROR(uefi_call_wrapper(st->ConIn->ReadKeyStroke,2,st->ConIn,&k))){ if(k.ScanCode==SCAN_F12 || k.ScanCode==SCAN_PRINT){ if(!EFI_ERROR(save_screenshot_bmp(ih,st,g))){ UINTN rw; StrCpy(toast,L"Saved \\screenshot"); for(rw=0;rw<win_count;rw++) reload_window(ih,st,&wins[rw]); } else StrCpy(toast,L"Screenshot failed"); toast_ticks=120; need_redraw=TRUE; } else if(k.ScanCode==SCAN_DELETE && active>=0){ LAUNCHER_WINDOW *aw=&wins[active]; if(aw->sel<aw->count && StrCmp(aw->items[aw->sel].name,L"..")!=0){ if(!EFI_ERROR(delete_selected_with_meta(ih,st,aw->cwd,aw->items[aw->sel].name))){ reload_all_windows(ih,st,wins,win_count); if(aw->count==0) aw->sel=0; else if(aw->sel>=aw->count) aw->sel=aw->count-1; StrCpy(toast,L"Delete success"); toast_ticks=120; need_redraw=TRUE; } else { StrCpy(toast,L"Delete failed"); toast_ticks=120; need_redraw=TRUE; } } } } }
 ...
 if(ps.RelativeMovementZ!=0){ INTN wi; for(wi=(INTN)win_count-1;wi>=0;wi--){ LAUNCHER_WINDOW *w=&wins[wi]; if(px>=(INTN)w->x&&px<(INTN)(w->x+w->w)&&py>=(INTN)w->y&&py<(INTN)(w->y+w->h)){ wi=bring_window_to_front(wins,win_count,wi); active=wi; w=&wins[wi]; need_redraw=TRUE; UINTN old_scroll=w->scroll; if(ps.RelativeMovementZ>0){ if(w->scroll<w->max_scroll) w->scroll++; } else { if(w->scroll>0) w->scroll--; } if(w->scroll!=old_scroll){ pr_old_x=w->x; pr_old_y=w->y; pr_old_w=w->w; pr_old_h=w->h; pr_new_x=w->x; pr_new_y=w->y; pr_new_w=w->w; pr_new_h=w->h; partial_redraw=TRUE; } break; } } } }
 if(ps.LeftButton && ps.RightButton && !(prev_left && prev_right)){ INTN wi; meta_popup=FALSE; meta_popup_pinned=FALSE; meta_popup_pin_armed=FALSE; for(wi=(INTN)win_count-1;wi>=0;wi--){ LAUNCHER_WINDOW *w=&wins[wi]; if(px>=(INTN)w->x&&px<(INTN)(w->x+w->w)&&py>=(INTN)w->y&&py<(INTN)(w->y+w->h)){ wi=bring_window_to_front(wins,win_count,wi); active=wi; w=&wins[wi]; need_redraw=TRUE; UINTN start=w->scroll*w->cols,end=start+w->visible_rows*w->cols,j; if(end>w->count) end=w->count; for(j=start;j<end;j++){ UINTN vi=j-start,row=vi/w->cols,col=vi%w->cols,ix=w->x+12+col*90+26/2,iy=w->content_y+row*83; if(px>=(INTN)(ix-6)&&px<(INTN)(ix+ICON_PX+6)&&py>=(INTN)(iy-6)&&py<(INTN)(iy+ICON_PX+22)){ read_meta_preview(ih,st,w->cwd,w->items[j].name,meta_popup_text,sizeof(meta_popup_text)/sizeof(CHAR16)); meta_popup=TRUE; meta_popup_edit_mode=FALSE; meta_popup_dirty=FALSE; StrCpy(meta_popup_cwd,w->cwd); StrCpy(meta_popup_name,w->items[j].name); meta_popup_cursor=StrLen(meta_popup_text); meta_popup_x=px+10; meta_popup_y=py+10; meta_popup_open_px=px; meta_popup_open_py=py; meta_popup_pin_armed=TRUE; need_redraw=TRUE; break; } } break; } } }
 if(meta_popup && meta_popup_pin_armed && !meta_popup_pinned){
     INTN dx=px-meta_popup_open_px,dy=py-meta_popup_open_py; UINTN adx=(dx<0)?(UINTN)(-dx):(UINTN)dx,ady=(dy<0)?(UINTN)(-dy):(UINTN)dy;
     if(adx>=META_POPUP_PIN_THRESHOLD_PX || ady>=META_POPUP_PIN_THRESHOLD_PX){
         UINTN li=0,pos=0,pw=180,ph=48,px2=(meta_popup_x<0)?0:(UINTN)meta_popup_x,py2=(meta_popup_y<0)?0:(UINTN)meta_popup_y;
         CHAR16 lines[6][96]; ZeroMem(lines,sizeof(lines));
         while(li<6 && meta_popup_text[pos]){ pos=meta_preview_line(meta_popup_text,pos,lines[li],96); li++; }
         compute_meta_popup_bounds(&ttfont,meta_popup_text,meta_popup_x,meta_popup_y,rt.w,rt.h,&px2,&py2,&pw,&ph);
         if(px>=(INTN)px2&&px<(INTN)(px2+pw)&&py>=(INTN)py2&&py<(INTN)(py2+ph)){ meta_popup_pinned=TRUE; meta_popup_pin_armed=FALSE; }
     }
 }
 if((!ps.LeftButton || !ps.RightButton) && (prev_left && prev_right) && meta_popup && !meta_popup_pinned){ meta_popup=FALSE; meta_popup_pin_armed=FALSE; need_redraw=TRUE; }
 if((ps.LeftButton ^ ps.RightButton) && !prev_left && !prev_right && meta_popup && meta_popup_pinned){ UINTN pw=0,ph=0,px2=0,py2=0; compute_meta_popup_bounds(&ttfont,meta_popup_text,meta_popup_x,meta_popup_y,rt.w,rt.h,&px2,&py2,&pw,&ph); if(px>=(INTN)px2&&px<(INTN)(px2+pw)&&py>=(INTN)py2&&py<(INTN)(py2+ph)){ meta_popup_edit_mode=TRUE; meta_popup_cursor=StrLen(meta_popup_text); need_redraw=TRUE; } else { if(meta_popup_edit_mode && meta_popup_dirty){ EFI_STATUS ms=write_meta_popup_text(ih,st,meta_popup_cwd,meta_popup_name,meta_popup_text); if(!EFI_ERROR(ms)){ StrCpy(toast,L"Metadata saved"); meta_popup_dirty=FALSE; meta_popup=FALSE; meta_popup_pinned=FALSE; meta_popup_pin_armed=FALSE; meta_popup_edit_mode=FALSE; } else { StrCpy(toast,L"Metadata save failed"); } toast_ticks=120; } else { meta_popup=FALSE; meta_popup_pinned=FALSE; meta_popup_pin_armed=FALSE; meta_popup_edit_mode=FALSE; } need_redraw=TRUE; } }
 if((ps.LeftButton ^ ps.RightButton) && !prev_left && !prev_right){ INTN wi; UINTN tmp; for(wi=(INTN)win_count-1;wi>=0;wi--){ LAUNCHER_WINDOW *w=&wins[wi]; if(px>=(INTN)w->x&&px<(INTN)(w->x+w->w)&&py>=(INTN)w->y&&py<(INTN)(w->y+w->h)){ wi=bring_window_to_front(wins,win_count,wi); active=wi; w=&wins[wi]; need_redraw=TRUE; if(rename_mode && active>=0){ UINTN st2=w->scroll*w->cols,en2=st2+w->visible_rows*w->cols; if(en2>w->count) en2=w->count; if(w->sel>=st2 && w->sel<en2){ UINTN vi3=w->sel-st2,row3=vi3/w->cols,col3=vi3%w->cols,ix3=w->x+12+col3*90+26/2,iy3=w->content_y+row3*83,lx3=ix3,ly3=iy3+ICON_PX+4; if(px>=(INTN)lx3&&px<(INTN)(lx3+ICON_PX)&&py>=(INTN)ly3&&py<(INTN)(ly3+16)){ UINTN rel=(px>(INTN)lx3)?(UINTN)(px-(INTN)lx3):0; rename_cursor=rename_click_to_cursor(&ttfont,rename_buf,rel); rename_all_selected=FALSE; need_redraw=TRUE; } else { rename_mode=FALSE; rename_all_selected=FALSE; } } else { rename_mode=FALSE; rename_all_selected=FALSE; } } if(win_count>1 && py<(INTN)(w->y+30) && px>=(INTN)(w->x+w->w-22) && px<(INTN)(w->x+w->w-10)){ free_window_icons(st,w); for(tmp=(UINTN)wi;tmp+1<win_count;tmp++) wins[tmp]=wins[tmp+1]; if(win_count>1) win_count--; if(active>=(INTN)win_count) active=(INTN)win_count-1; need_redraw=TRUE; break; }
 ...
 if(hit!=(UINTN)-1){ w->sel=hit; if(StrCmp(w->items[hit].name,L"..")!=0){ UINTN vi2=hit-start,row2=vi2/w->cols,col2=vi2%w->cols,ix2=w->x+12+col2*90+26/2,iy2=w->content_y+row2*83; pending_item_drag=TRUE; item_dragging=FALSE; drag_copy=(ps.RightButton && !ps.LeftButton)?TRUE:FALSE; drag_src_win=wi; drag_item=hit; drag_start_px=px; drag_start_py=py; drag_hot_x=px-(INTN)ix2; drag_hot_y=py-(INTN)iy2; if(drag_hot_x<0||drag_hot_x>(INTN)ICON_PX) drag_hot_x=ICON_PX/2; if(drag_hot_y<0||drag_hot_y>(INTN)ICON_PX) drag_hot_y=ICON_PX/2; } if(!EFI_ERROR(uefi_call_wrapper(st->RuntimeServices->GetTime,2,&t,NULL))){ ms=(UINTN)((((UINTN)t.Minute*60u)+(UINTN)t.Second)*1000u) + (UINTN)(t.Nanosecond/1000000u); have_ms=TRUE; }
 if(w->last_sel==hit && have_ms && (ms-w->last_click_ms<450)){ pending_item_drag=FALSE; item_dragging=FALSE; drag_src_win=-1; drag_item=(UINTN)-1; UINTN vi=hit-start,row=vi/w->cols,col=vi%w->cols,ix=w->x+12+col*90+26/2,iy=w->content_y+row*83; UINTN lx=ix,ly=iy+ICON_PX+4,lw=ICON_PX,lh=16; if(px>=(INTN)lx&&px<(INTN)(lx+lw)&&py>=(INTN)ly&&py<(INTN)(ly+lh) && StrCmp(w->items[hit].name,L"..")!=0){ rename_mode=TRUE; StrCpy(rename_buf,w->items[hit].name); rename_cursor=StrLen(rename_buf); rename_all_selected=TRUE; need_redraw=TRUE; } else if(w->items[hit].is_dir){ if(StrCmp(w->items[hit].name,L"..")!=0 && win_count<MAX_WINDOWS){ LAUNCHER_WINDOW *nw=&wins[win_count++]; ZeroMem(nw,sizeof(*nw)); nw->used=TRUE; join_path(nw->cwd,sizeof(nw->cwd),w->cwd,w->items[hit].name); nw->w=w->w; nw->h=w->h; nw->x=w->x+24; nw->y=w->y+24; if(nw->x+nw->w>g->Mode->Info->HorizontalResolution) nw->x=g->Mode->Info->HorizontalResolution-nw->w; if(nw->y+nw->h>g->Mode->Info->VerticalResolution) nw->y=g->Mode->Info->VerticalResolution-nw->h; layout_window(nw); reload_window(ih,st,nw); active=(INTN)win_count-1; }
 ...
 if(item_dragging && drag_src_win>=0 && drag_src_win<(INTN)win_count){
     ...
     for(dwi=(INTN)win_count-1; dwi>=0; dwi--){ LAUNCHER_WINDOW *dw=&wins[dwi];
         if(px>=(INTN)dw->x&&px<(INTN)(dw->x+dw->w)&&py>=(INTN)dw->y&&py<(INTN)(dw->y+dw->h)){
             if(drag_item<sw->count){
                if(drag_hover_item!=(UINTN)-1 && drag_hover_item<dw->count && dw->items[drag_hover_item].is_dir){
                    CHAR16 target_dir[256];
                    if(StrCmp(dw->items[drag_hover_item].name,L"..")==0){
                        UINTN len;
                        StrnCpy(target_dir,dw->cwd,255); target_dir[255]=0;
                        len=StrLen(target_dir);
                        if(len>1){
                            while(len>1 && target_dir[len-1]!=L'\\') len--;
                            if(len<=1) target_dir[1]=0; else target_dir[len-1]=0;
                        }
                    } else {
     ...
     if(ms2==EFI_ALREADY_STARTED) StrCpy(toast,drag_copy?L"Copy skipped":L"Move skipped");
     else if(!EFI_ERROR(ms2)){ reload_all_windows(ih,st,wins,win_count); StrCpy(toast,drag_copy?L"Copy success":L"Move success"); }
     else if(ms2!=EFI_NOT_FOUND) StrCpy(toast,drag_copy?L"Copy failed":L"Move failed");
     if(ms2!=EFI_NOT_FOUND){ toast_ticks=120; }
     need_redraw=TRUE;
 }
 item_dragging=FALSE; drag_copy=FALSE; drag_src_win=-1; drag_item=(UINTN)-1; drag_hover_item=(UINTN)-1;
 }
 if(((ps.LeftButton && !ps.RightButton) || (ps.RightButton && !ps.LeftButton)) && pending_item_drag && !item_dragging){ INTN dx=px-drag_start_px,dy=py-drag_start_py; if(dx<0) dx=-dx; if(dy<0) dy=-dy; if(dx+dy>=6){ pending_item_drag=FALSE; item_dragging=TRUE; need_redraw=TRUE; } }
 if(item_dragging){ INTN old_hover=drag_hover_win; UINTN old_folder=drag_hover_item; drag_hover_win=-1; drag_hover_item=(UINTN)-1; { INTN dwi; for(dwi=(INTN)win_count-1; dwi>=0; dwi--){ LAUNCHER_WINDOW *dw=&wins[dwi]; UINTN start,end,j; if(px<(INTN)dw->x||px>=(INTN)(dw->x+dw->w)||py<(INTN)dw->y||py>=(INTN)(dw->y+dw->h)) continue; drag_hover_win=dwi; start=dw->scroll*dw->cols; end=start+dw->visible_rows*dw->cols; if(end>dw->count) end=dw->count; for(j=start;j<end;j++){ UINTN vi=j-start,row=vi/dw->cols,col=vi%dw->cols,ix=dw->x+12+col*90+26/2,iy=dw->content_y+row*83; if(px>=(INTN)(ix-6)&&px<(INTN)(ix+ICON_PX+6)&&py>=(INTN)(iy-6)&&py<(INTN)(iy+ICON_PX+22) && dw->items[j].is_dir){ if(!((INTN)j==(INTN)drag_item && dwi==drag_src_win)) drag_hover_item=j; break; } } break; } } if(drag_hover_win!=old_hover || drag_hover_item!=old_folder || (INTN)px!=ppx || (INTN)py!=ppy) need_redraw=TRUE; } else if(drag_hover_win!=-1 || drag_hover_item!=(UINTN)-1){ drag_hover_win=-1; drag_hover_item=(UINTN)-1; need_redraw=TRUE; }
 if(ps.LeftButton && !ps.RightButton && dragging && active>=0){ LAUNCHER_WINDOW *w=&wins[active]; UINTN ox0=w->x,oy0=w->y,ow0=w->w,oh0=w->h; INTN nx=px-drag_dx,ny=py-drag_dy; if(nx<0)nx=0; if(ny<0)ny=0; if((UINTN)nx+w->w>g->Mode->Info->HorizontalResolution) nx=(INTN)(g->Mode->Info->HorizontalResolution-w->w); if((UINTN)ny+w->h>g->Mode->Info->VerticalResolution) ny=(INTN)(g->Mode->Info->VerticalResolution-w->h); if((UINTN)nx!=w->x || (UINTN)ny!=w->y){ pr_old_x=ox0; pr_old_y=oy0; pr_old_w=ow0; pr_old_h=oh0; w->x=(UINTN)nx; w->y=(UINTN)ny; layout_window(w); pr_new_x=w->x; pr_new_y=w->y; pr_new_w=w->w; pr_new_h=w->h; partial_redraw=TRUE; } }
 if(ps.LeftButton && !ps.RightButton && resizing && active>=0 && resize_edge){ LAUNCHER_WINDOW *w=&wins[active]; UINTN ox0=w->x,oy0=w->y,ow0=w->w,oh0=w->h; INTN nw=(INTN)ow + (px-(INTN)(ox+ow-1)); INTN nh=(INTN)oh + (py-(INTN)(oy+oh-1)); if(nw<(INTN)MIN_W) nw=MIN_W; if(nh<(INTN)MIN_H) nh=MIN_H; if((UINTN)w->x+(UINTN)nw>g->Mode->Info->HorizontalResolution) nw=(INTN)(g->Mode->Info->HorizontalResolution-w->x); if((UINTN)w->y+(UINTN)nh>g->Mode->Info->VerticalResolution) nh=(INTN)(g->Mode->Info->VerticalResolution-w->y); if((UINTN)nw!=w->w || (UINTN)nh!=w->h){ pr_old_x=ox0; pr_old_y=oy0; pr_old_w=ow0; pr_old_h=oh0; w->w=(UINTN)nw; w->h=(UINTN)nh; layout_window(w); pr_new_x=w->x; pr_new_y=w->y; pr_new_w=w->w; pr_new_h=w->h; partial_redraw=TRUE; } }
