...
EFI_STATUS efi_main(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st){ EFI_STATUS s; RENDER_TARGET rt; EFI_GRAPHICS_OUTPUT_PROTOCOL *g=NULL; EFI_GUID gg=gEfiGraphicsOutputProtocolGuid; EFI_SIMPLE_POINTER_PROTOCOL *sp=NULL; EFI_GUID spg=EFI_SIMPLE_POINTER_PROTOCOL_GUID; EFI_ABSOLUTE_POINTER_PROTOCOL *ap=NULL; EFI_GUID apg=EFI_ABSOLUTE_POINTER_PROTOCOL_GUID; EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *inex=NULL; EFI_GUID inexg=EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL_GUID; EFI_INPUT_KEY k; TT_FONT ttfont; EFI_EVENT events[4]; UINTN evn=0,which=0; POINTER_IMAGE ptr_img; ICON_IMAGE close_img; EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ptr_bg=NULL; UINTN ptr_w=12,ptr_h=14; BOOLEAN ptr_bg_valid=FALSE; INTN px=200,py=140,ppx=-1,ppy=-1; UINT64 last_abs_x=0,last_abs_y=0; UINT32 last_abs_buttons=0; BOOLEAN prev_left=FALSE,need_redraw=TRUE;
 ...
 if(have_ms) w->last_click_ms=ms; w->last_sel=hit; pr_old_x=w->x; pr_old_y=w->y; pr_old_w=w->w; pr_old_h=w->h; pr_new_x=w->x; pr_new_y=w->y; pr_new_w=w->w; pr_new_h=w->h; partial_redraw=TRUE; }
 }
 break; } } }
 ...
 for(i=0;i<win_count;i++) free_window_icons(st,&wins[i]); if(rt.buf) FreePool(rt.buf); if(ttfont.data) uefi_call_wrapper(st->BootServices->FreePool,1,ttfont.data); if(ptr_bg) FreePool(ptr_bg); if(ptr_img.px) FreePool(ptr_img.px); if(close_img.px) uefi_call_wrapper(st->BootServices->FreePool,1,close_img.px); return EFI_SUCCESS;
}
