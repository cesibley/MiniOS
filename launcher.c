...
EFI_STATUS efi_main(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st){ EFI_STATUS s; RENDER_TARGET rt; EFI_GRAPHICS_OUTPUT_PROTOCOL *g=NULL; EFI_GUID gg=gEfiGraphicsOutputProtocolGuid; EFI_SIMPLE_POINTER_PROTOCOL *sp=NULL; EFI_GUID spg=EFI_SIMPLE_POINTER_PROTOCOL_GUID; EFI_ABSOLUTE_POINTER_PROTOCOL *ap=NULL; EFI_GUID apg=EFI_ABSOLUTE_POINTER_PROTOCOL_GUID; EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *inex=NULL; EFI_GUID inexg=EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL_GUID; EFI_INPUT_KEY k; TT_FONT ttfont; EFI_EVENT events[4]; UINTN evn=0,which=0; POINTER_IMAGE ptr_img; ICON_IMAGE close_img; EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ptr_bg=NULL; UINTN ptr_w=12,ptr_h=14; BOOLEAN ptr_bg_valid=FALSE; INTN px=200,py=140,ppx=-1,ppy=-1; BOOLEAN prev_left=FALSE,need_redraw=TRUE,rename_mode=FALSE,rename_all_selected=FALSE; CHAR16 rename_buf[LAUNCHER_NAME_MAX]; UINTN rename_cursor=0; CHAR16 toast[64]; UINTN toast_ticks=0; BOOLEAN meta_popup=FALSE; CHAR16 meta_popup_text[192]; INTN meta_popup_x=0,meta_popup_y=0; BOOLEAN prev_right=FALSE;
     ...
     for(dwi=(INTN)win_count-1; dwi>=0; dwi--){ LAUNCHER_WINDOW *dw=&wins[dwi];
         if(px>=(INTN)dw->x&&px<(INTN)(dw->x+dw->w)&&py>=(INTN)dw->y&&py<(INTN)(dw->y+dw->h)){
             if(drag_item<sw->count){
                if(drag_hover_folder!=(UINTN)-1 && drag_hover_folder<dw->count && dw->items[drag_hover_folder].is_dir){
                    CHAR16 target_dir[256];
                    if(StrCmp(dw->items[drag_hover_folder].name,L"..")==0){
                        UINTN len;
                        ...
                        join_path(target_dir,sizeof(target_dir),dw->cwd,dw->items[drag_hover_folder].name);
                    }
 ...
 item_dragging=FALSE; drag_src_win=-1; drag_item=(UINTN)-1; drag_hover_folder=(UINTN)-1;
 }
 ...
 if(item_dragging){ INTN old_hover=drag_hover_win; UINTN old_folder=drag_hover_folder; drag_hover_win=-1; drag_hover_folder=(UINTN)-1; { INTN dwi; for(dwi=(INTN)win_count-1; dwi>=0; dwi--){ LAUNCHER_WINDOW *dw=&wins[dwi]; UINTN start,end,j; if(px<(INTN)dw->x||px>=(INTN)(dw->x+dw->w)||py<(INTN)dw->y||py>=(INTN)(dw->y+dw->h)) continue; drag_hover_win=dwi; start=dw->scroll*dw->cols; end=start+dw->visible_rows*dw->cols; if(end>dw->count) end=dw->count; for(j=start;j<end;j++){ UINTN vi=j-start,row=vi/dw->cols,col=vi%dw->cols,ix=dw->x+12+col*90+26/2,iy=dw->content_y+row*83; if(px>=(INTN)(ix-6)&&px<(INTN)(ix+ICON_PX+6)&&py>=(INTN)(iy-6)&&py<(INTN)(iy+ICON_PX+22) && dw->items[j].is_dir){ if(!((INTN)j==drag_item && dwi==drag_src_win)) drag_hover_folder=j; break; } } break; } } if(drag_hover_win!=old_hover || drag_hover_folder!=old_folder || (INTN)px!=ppx || (INTN)py!=ppy) need_redraw=TRUE; } else if(drag_hover_win!=-1 || drag_hover_folder!=(UINTN)-1){ drag_hover_win=-1; drag_hover_folder=(UINTN)-1; need_redraw=TRUE; }
