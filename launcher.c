...
EFI_STATUS efi_main(EFI_HANDLE ih, EFI_SYSTEM_TABLE *st){ EFI_STATUS s; EFI_GRAPHICS_OUTPUT_PROTOCOL *g=NULL; EFI_GUID gg=gEfiGraphicsOutputProtocolGuid; EFI_SIMPLE_POINTER_PROTOCOL *sp=NULL; EFI_GUID spg=EFI_SIMPLE_POINTER_PROTOCOL_GUID; EFI_ABSOLUTE_POINTER_PROTOCOL *ap=NULL; EFI_GUID apg=EFI_ABSOLUTE_POINTER_PROTOCOL_GUID; EFI_INPUT_KEY k; TT_FONT ttfont; EFI_EVENT events[3]; UINTN evn=0,which=0; POINTER_IMAGE ptr_img; EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ptr_bg=NULL; UINTN ptr_w=12,ptr_h=14; BOOLEAN ptr_bg_valid=FALSE; INTN px=200,py=140,ppx=-1,ppy=-1; UINT64 last_abs_x=0,last_abs_y=0; UINT32 last_abs_buttons=0; BOOLEAN prev_left=FALSE,need_redraw=TRUE;
 ...
 LAUNCHER_WINDOW wins[MAX_WINDOWS]; UINTN win_count=1; INTN active=0; UINTN i; BOOLEAN dragging=FALSE,resizing=FALSE; INTN drag_dx=0,drag_dy=0; UINTN resize_edge=0,ox=0,oy=0,ow=0,oh=0;
 ...
 while(1){ if(need_redraw){ UINTN widx; fill(g,0,0,g->Mode->Info->HorizontalResolution,g->Mode->Info->VerticalResolution,desk); for(widx=0;widx<win_count;widx++){ LAUNCHER_WINDOW *w=&wins[widx]; UINTN start=w->scroll*w->cols,end=start+w->visible_rows*w->cols,j; if(end>w->count) end=w->count; fill(g,w->x,w->y,w->w,w->h,win); fill(g,w->x,w->y,w->w,30,title); { CHAR16 ttl[80]; SPrint(ttl,sizeof(ttl),L"Launcher - %s",w->cwd); draw_tt_text(g,&ttfont,ttl,(int)(w->x+12),(int)(w->y+7),white,title);} fill(g,w->x+w->w-22,w->y+7,12,12,btn); draw_tt_text(g,&ttfont,L"X",(int)(w->x+w->w-19),(int)(w->y+6),white,btn);
 ...
 need_redraw=FALSE; }
 if(ptr_bg_valid && ppx>=0 && ppy>=0){ uefi_call_wrapper(g->Blt,10,g,ptr_bg,EfiBltBufferToVideo,0,0,(UINTN)ppx,(UINTN)ppy,ptr_w,ptr_h,0); ptr_bg_valid=FALSE; }
 ...
 if(have_ms) w->last_click_ms=ms; w->last_sel=hit; need_redraw=TRUE; }
 }
 break; } } }
 if(!ps.LeftButton){ dragging=FALSE; resizing=FALSE; }
 if(ps.LeftButton && dragging && active>=0){ LAUNCHER_WINDOW *w=&wins[active]; INTN nx=px-drag_dx,ny=py-drag_dy; if(nx<0)nx=0; if(ny<0)ny=0; if((UINTN)nx+w->w>g->Mode->Info->HorizontalResolution) nx=(INTN)(g->Mode->Info->HorizontalResolution-w->w); if((UINTN)ny+w->h>g->Mode->Info->VerticalResolution) ny=(INTN)(g->Mode->Info->VerticalResolution-w->h); if((UINTN)nx!=w->x || (UINTN)ny!=w->y){ w->x=(UINTN)nx; w->y=(UINTN)ny; layout_window(w); need_redraw=TRUE; } }
 if(ps.LeftButton && resizing && active>=0 && resize_edge){ LAUNCHER_WINDOW *w=&wins[active]; INTN nw=(INTN)ow + (px-(INTN)(ox+ow-1)); INTN nh=(INTN)oh + (py-(INTN)(oy+oh-1)); if(nw<(INTN)MIN_W) nw=MIN_W; if(nh<(INTN)MIN_H) nh=MIN_H; if((UINTN)w->x+(UINTN)nw>g->Mode->Info->HorizontalResolution) nw=(INTN)(g->Mode->Info->HorizontalResolution-w->x); if((UINTN)w->y+(UINTN)nh>g->Mode->Info->VerticalResolution) nh=(INTN)(g->Mode->Info->VerticalResolution-w->y); if((UINTN)nw!=w->w || (UINTN)nh!=w->h){ w->w=(UINTN)nw; w->h=(UINTN)nh; layout_window(w); need_redraw=TRUE; } }
